...
shared_examples_for 'bulk updatable model' do |use_prepared_statements|
  let(:use_prepared_statements) { type }
  DATA_TYPES = %i[
    bigint
    bit
    boolean
  ...
  before do
    allow(BulkUpdatableModel.connection)
      .to receive(:prepared_statements).and_return(use_prepared_statements || false)
  end
